...
def CalcularEmprestimo(valor_emprestimo, quantidade_parcelas, idade_pessoa):    
    ...
    mes_corrente = 1
    valor_parcela_corrigido = 0
    valor_total = 0    

    #é possível remover o while, deste método. Isso fica como exercicío. Dê uma pesquisada sobre soma de termos em uma PA
    while(mes_corrente <= quantidade_parcelas):
        ...
        valor_total += valor_parcela_corrigido
        mes_corrente += 1 #esta expressão equivale a: mes_corrente = mes_corrente + 1
    
    return valor_total, valor_parcela_corrigido #esta é a forma de retornar mais de um valor da função.
...
if PessoaAptaParcelamento(idade_pessoa, quantidade_parcelas):
    valor_total_emprestimo, valor_ultima_parcela_corrigida = CalcularEmprestimo(valor_emprestimo, quantidade_parcelas, idade_pessoa)
